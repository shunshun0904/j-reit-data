...
- ワークフローは手動実行のみ（workflow_dispatch）。スケジュール実行はしない
  ...
  - `inspect-reit-site.yml` 投資法人サイトのポートフォリオ一覧の構造確認と要約（個人利用。KDX のみ実装）。
    ...
    `mode=distribution` はトップページから「分配金」リンクを辿って分配金ページを読み、J-Quants cache の
    DPU 履歴から分割調整後の変化率だけを出す（金額は出さない）。
    確認済み（2026-09-21）: KDX の分配金ページ（`/ja/ir/distribute.html`）は HTML 表を持たず
    （注記の表のみ）`read_html` では読めない。J-Quants の 8972 は 20 期（2016-10〜2026-04）、
    分割 2 回の換算で不連続なし（換算が正しい根拠）、換算後 DPU は 9.5 年で +46.6%（年率 +4.1%）、
    前期比で増配 15 回・減配 2 回（2019-10 −0.6%, 2021-10 −9.0%）
